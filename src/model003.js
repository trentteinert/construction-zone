...
import Model003 from './components/Model003';

export default function Model003page() {
  ...
  return (
    <Wrapper className="App">
      <Canvas clasName="canvas" camera={{ fov: 40, position: [5, 3, 0] }}>
        <OrbitControls target={[0, 0.8, 0]} />
        <ambientLight intensity={0.5} />
        <spotLight position={[3, 3, 1]} angle={0.3} />
        <Suspense fallback={null}>
          <Model003 position={[0, 0, 0]} />
        </Suspense>
      </Canvas>
        <div>
